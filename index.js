#! /usr/bin/env node
console.log(chalk.bold.bgWhiteBright.magentaBright(`\n\t\t\tWelcome To Simple Word And Character Counter: `));
...
if (options.select === "Word count") {
    ...
    console.log(chalk.yellowBright(`Your input contains ${words.length} words and ${characters.length} characters.`));
}
else if (options.select === "Info") {
    console.log(chalk.bold.yellowBright(`\tAuthor: Muhammad Nofil Shoaib.\n\tA simple Word and character counter made with TypeScript, JavaScript, Node.js and Inquirer.`));
}
